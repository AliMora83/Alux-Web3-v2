...
function ListingPage({}: Props) {
  ...
  const [minimumNextBid, setMinimumNextBid] =
    React.useState<{
      displayValue: string;
      symbol: string;
    }>();
  ...
  const fetchMinNextBid = async () => {
    ...
    const minBidResponse =
      await contract.auction.getMinimumNextBid(
        listingId
      );
  ...
  const getGalleryImages: any =
    React.useCallback(() => {
      const galleryImages = listing?.asset
  ...
  return (
    <div className="to-gary-100[0.35] dark:to-gray-300[0.25] bg-gradient-to-tr from-gray-300/[0.35] dark:from-purple-500/[0.15] min-h-screen pb-10 md:pb-10">
      ...
      {!listing && !isLoading && (
        <h1 className="">Listing not fount</h1>
      )}
          ...
          <section className="flex-1 space-y-5">
            <div className="">
              ...
              <div className="flex items-center">
                ...
                <a
                  href={`https://maps.google.com/?q=${listing.asset.name}`}
                  target="_blank"
                >
