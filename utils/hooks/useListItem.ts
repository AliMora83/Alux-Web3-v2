import { useActiveListings, useContract, useCreateAuctionListing, useCreateDirectListing, useNetwork, useNetworkMismatch } from "@thirdweb-dev/react";
...
function useListItem() {
  ...
  const [listingType, setListingType] = React.useState<"directListing" | "auctionListing" | null>(null);
  ...
  const { data: listings, isLoading: loadingListings, refetch: refetchListing } = useActiveListings(contract);

  const networkMismatch = useNetworkMismatch();
  const [, switchNetwork] = useNetwork();

  const { mutate: createDirectListing, isLoading: isDirectListingLoading, error: directListingError } = useCreateDirectListing(contract);
  const { mutate: createAuctionListing, isLoading: isAuctionListingLoading, error: auctionListingError } = useCreateAuctionListing(contract);
